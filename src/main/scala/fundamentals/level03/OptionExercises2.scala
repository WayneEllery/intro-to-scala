...
object OptionExercises2 {
  ...
  /**
    ...
    * ```
    */
  def findJobDescriptionGivenJobId2(jobId: JobId): Option[String] = findJobById(jobId).map(j => j.description)
  ...
  /**
    ...
    */
  def findJobIdByHumanId(humanId: HumanId): Option[JobId] = findHumanById(humanId).flatMap(h => h.optJobId)
